...
session and logs are under `build/fr/<key>/`. The failing cases, first (each row from the latest run of that board on the committed wrapper, 2026-09-24 07:30 UTC):
...
digest, which every gate row prints. Four rungs are green (D69): the smoke test, `open-book-c1`, `olimex-esp32c3-devkit`, `olimex-rp2040-pico-pc`, in 15 to 265 s each. The next rung is `crkbd-corne-cherry`, then `libresolar-mppt-2420`, both killed at
...
*State (2026-09-24, 07:30 UTC):* gate FAIL, 4 of 6. Stage 5's baseline (Freerouting 2.4.1, optimiser off,
...
violations each, in under a minute a board but rp2040's four and a half; `crkbd-corne-cherry` and
`libresolar-mppt-2420` were killed at the 20-minute cap with no session file, before the optimiser was off.
Numbers per board in the next-step section's table. Tests: the class A suite, 86 passed in 41 s (the parked
