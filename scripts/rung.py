...
for iterating on a rung under a short budget (D77), where `gate.py b <key>` is the verdict.
    ...
    python3 scripts/rung.py <key> <mode> <passes> <timeout_s> [stubs] [fanout] [feeds] [loose|after|reserved|vias] [inpad] [rounds=N] [open=<board>]
...
unknown = set(options) - {"stubs", "fanout", "feeds", "loose", "after", "reserved", "vias", "inpad", "rounds", "open", "exits"}
if unknown:
