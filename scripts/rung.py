...
for iterating on a rung under a short budget (D77), where `gate.py b <key>` is the verdict.
    ...
    python3 scripts/rung.py <key> <mode> <passes> <timeout_s> [stubs] [fanout] [feeds] [loose|after|reserved|vias|stitch] [inpad] [pourpins] [costs=L:C,...] [band=REF:IN:OUT,...] [rounds=N] [open=<board>]
...
unknown = set(options) - {"stubs", "fanout", "feeds", "loose", "after", "reserved", "vias", "stitch", "inpad", "pourpins", "costs", "band", "rounds", "open", "exits"}
if unknown:
...
work = refs.repo_root() / "build" / "fr" / (f"{key}-{mode}{'-stubs' if stubs else ''}{'-fanout' if fanout else ''}"
                                             ...
                                             f"{'-costs' if layer_costs else ''}{'-band' if via_bands else ''}"
